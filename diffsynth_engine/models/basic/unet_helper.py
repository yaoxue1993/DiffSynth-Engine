...
class BasicTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_attention_heads, attention_head_dim, cross_attention_dim,
        ...
        self.attn2 = Attention(q_dim=dim, kv_dim=cross_attention_dim, num_heads=num_attention_heads,
                               head_dim=attention_head_dim, bias_out=True, device=device, dtype=dtype)
...
class AttentionBlock(nn.Module):
    def __init__(self, num_attention_heads, attention_head_dim, in_channels, num_layers=1, cross_attention_dim=None,
                 norm_num_groups=32, eps=1e-5, need_proj_out=True,
                 device: str = 'cuda:0', dtype: torch.dtype = torch.float16):
        super().__init__()
