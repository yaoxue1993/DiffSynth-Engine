...
class LoRALinear(nn.Linear):
    ...
    def from_linear(linear: nn.Linear):        
        lora_linear = torch.nn.utils.skip_init(LoRALinear, 
                                               linear.in_features, 
                                               linear.out_features, 
...
class LoRAConv2d(nn.Conv2d):
    ...
    def from_conv2d(conv2d: nn.Conv2d):
        lora_conv2d = torch.nn.utils.skip_init(LoRAConv2d, 
                                               conv2d.in_channels, 
                                               conv2d.out_channels, 
                                               conv2d.kernel_size, 
                                               conv2d.stride, 
    ...
    def _construct_lora(self, name: str, scale: float, rank: int, alpha: int, up: torch.Tensor, down: torch.Tensor, device: str, dtype: torch.dtype):
        down_conv = torch.nn.utils.skip_init(nn.Conv2d, 
                                            self.in_channels, 
                                            rank,
