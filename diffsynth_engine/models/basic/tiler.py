...
class TileWorker:
    ...
    def tiled_inference(self, forward_fn, model_input, tile_batch_size, inference_device, inference_dtype,
                        tile_device, tile_dtype):
    ...
    def tiled_forward(self, forward_fn, model_input, tile_size, tile_stride, tile_batch_size=1,
                      tile_device="cpu", tile_dtype=torch.float32, border_width=None):
        # Prepare
        ...
        # inference
        model_output = self.tiled_inference(forward_fn, model_input, tile_batch_size, inference_device, inference_dtype,
                                            tile_device, tile_dtype)
        ...
        # untile
        model_output = self.untile(model_output, height, width, tile_size, tile_stride, border_width,
                                   tile_device, tile_dtype)
...
class FastTileWorker:
    ...
    def tiled_forward(self, forward_fn, model_input, tile_size, tile_stride,
                      tile_device="cpu", tile_dtype=torch.float32, border_width=None):
        # Prepare
...
class TileWorker2Dto3D:
    ...
    def tiled_forward(
        ...
        value = torch.zeros((B, int(C * scale_C), int(T * scale_T), int(H * scale_H), int(W * scale_W)),
                            dtype=tile_dtype, device=tile_device)
        weight = torch.zeros((1, 1, int(T * scale_T), int(H * scale_H), int(W * scale_W)),
                             dtype=tile_dtype, device=tile_device)
        ...
        for h in range(0, H, tile_stride_H):
            for w in range(0, W, tile_stride_W):
                if ((h - tile_stride_H >= 0 and h - tile_stride_H + tile_size_H >= H) or
                        (w - tile_stride_W >= 0 and w - tile_stride_W + tile_size_W >= W)):
                    continue
        ...
        for hl, hr, wl, wr in progress_bar(tasks):
            ...
            value[:, :, :, int(hl * scale_H):int(hr * scale_H),
            int(wl * scale_W):int(wr * scale_W)] += grid_output * mask
