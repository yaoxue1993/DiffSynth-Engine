...
def fetch_civitai_model(model_url:str) -> str:
    ...
    if "/models/" in parsed_url.path:
        if parsed_url.query:
            ...
            result = requests.get(f"https://civitai.com/api/v1/models/{model_id}", headers={"Content-Type": "application/json"}).json()
    ...
    if os.path.exists(filepath):
        ...
        return filepath
    response = requests.get(download_url, stream=True, timeout=4 * 60 * 60, headers={"Content-Type": "application/json"}) # 4h
