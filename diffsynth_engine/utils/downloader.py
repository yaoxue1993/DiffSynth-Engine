...
from diffsynth_engine.configs.model_loader_config import preset_models_on_huggingface, preset_models_on_modelscope, Preset_model_id
...
def download_from_modelscope(model_id, origin_file_path, local_dir):
    ...
    if file_name in os.listdir(local_dir):
        logger.info(f"    {file_name} has been already in {local_dir}.")
    else:
        logger.info(f"    Start downloading {os.path.join(local_dir, file_name)}")
...
def download_models(
    ...
    logger.info(f"Downloading models: {model_id_list}")
    downloaded_files = []
    load_files = []
