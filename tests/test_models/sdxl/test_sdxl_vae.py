...
class TestSDXLVAE(ImageTestCase):
    ...
    def test_and_save_tensors(self):
        ...
        image_tensor = image_tensor.permute(2, 0, 1).unsqueeze(0).to("cuda:0")
        with torch.no_grad():
            expected_encoded = vae.quant_conv(vae.encoder(image_tensor))[:, : self.latent_channels]
            ...
            expected_decoded = vae.decoder(vae.post_quant_conv(latent))
            decoded = self.decoder(encoded)
            self.assertTensorEqual(decoded, expected_decoded)

        expect = {"encoded": encoded, "decoded": decoded}
        save_path = self.testdata_dir / "expect/sdxl/sdxl_vae.safetensors"
